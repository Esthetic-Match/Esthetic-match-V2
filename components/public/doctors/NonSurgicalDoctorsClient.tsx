...
export default function NonSurgicalDoctorsClient() {
  const t =
    useTranslations(
      "doctor.doctor.nonSurgicalSpecialists",
    );
  ...
  /*
   * Used only for the hero specialty chips.
   *
   * Doctor cards themselves already receive
   * their localized names directly from the
   * doctor-profile API.
   */
  const [
  ...
  const specialtyNameById =
    useMemo(() => {
      ...
      /*
       * Full catalogue values have priority.
       */
      for (
        const specialty of catalogueSpecialties
      ) {
        map.set(
          specialty.id,
          specialty.name,
      ...
      /*
       * Also collect translated specialty
       * names already returned with doctors.
       */
      for (
        const doctor of doctors
      ) {
        for (
          const specialty of doctor.specialties ??
          []
        ) {
          if (
            !map.has(
  ...
  /* ═══════════════════════════════════
     LOAD CATALOGUE SPECIALTY NAMES
  ═══════════════════════════════════ */
        ...
        try {
          ...
          setDoctors(
            (
              ...
              /*
               * Prevent duplicate cards if a doctor
               * appears across pagination boundaries.
               */
              const existingIds =
                new Set(
                  currentDoctors.map(
      ...
      <section className="relative overflow-hidden border-b border-[#CEB591]/20 bg-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-20 md:px-10 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end lg:py-24">
          <div>
            ...
            {/* Database-backed specialty names */}

            <div className="mt-7 flex flex-wrap gap-2">
              {NON_SURGICAL_SPECIALTIES.map(
                (
                  specialtyId,
                ) => (
                  <span
