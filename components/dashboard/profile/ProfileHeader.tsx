...
import SpecialtyModal from "../settings/modal/SpecialtyModal";
...
type ProfileHeaderProps = {
  ...
  onUpdateProfile: (
    data: Partial<
      Omit<DoctorProfileData, "id" | "userId" | "user">
    >
  ) => void;
};
...
export default function ProfileHeader({
  ...
  const [isSpecialtyModalOpen, setIsSpecialtyModalOpen] =
    useState(false);
  ...
  const [currentSpecialtyIds, setCurrentSpecialtyIds] =
    useState<string[]>(specialty ?? []);
    ...
    <>
      <section className="relative z-20 mx-auto -mt-16 w-[calc(100%-2rem)] max-w-6xl rounded-3xl border border-gray-300/10 bg-white px-6 pb-8 pt-24 shadow-lg md:-mt-20 md:px-10 md:pt-10">
        ...
        <div className="absolute -top-20 left-6 md:left-10">
          <div className="relative h-40 w-40 rounded-full border-4 border-white bg-white shadow-md">
            ...
            <button
              ...
              className="absolute bottom-1 right-2 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border border-black/10 bg-white text-[#283C5D] shadow-md transition hover:border-2 hover:border-white hover:bg-[#283c5d] hover:text-white active:scale-[0.96]"
        ...
        <div className="flex flex-col gap-8 md:ml-48 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2">
              <h1 className="break-words text-2xl font-bold tracking-tight text-[#283C5D] md:text-3xl">
                {name}
              </h1>
            ...
            <div className="mt-5 space-y-3">
              ...
              {workAddress ? (
                <div className="flex items-start gap-3 text-sm text-[#283C5D]/75">
                  <MapPin
                    size={17}
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    border border-black/10 bg-[#283C5D] text-white transition hover:border-[#d8bd8d] 
                    hover:bg-[#d8bd8d] hover:text-[#283C5D] active:scale-[0.96]"
                  >
                    <Pencil size={11} />
                  </button>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {currentSpecialtyIds.length > 0 ? (
                    currentSpecialtyIds.map(
                      (item: string) => (
                        <span
                          key={item}
          ...
          <div className="flex shrink-0 flex-col items-center gap-3">
            <Link
              ...
              className="inline-flex w-fit cursor-pointer items-center gap-2 rounded-full border border-[#283C5D]/10 bg-white px-6 py-2 text-xs font-medium text-[#283C5D] shadow-sm transition hover:bg-[#283C5D] hover:text-white active:scale-[0.98]"
            >
              <Pencil size={20} />
            ...
            <button
              ...
              disabled={!slug}
              className="inline-flex w-fit cursor-pointer items-center gap-2 rounded-full border border-[#283C5D]/10 bg-white px-6 py-2 text-xs font-medium text-[#283C5D] shadow-sm transition hover:bg-[#283C5D] hover:text-white active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
            >
              {copied ? (
                <Check size={18} />
              ) : (
        ...
        {isSpecialtyModalOpen ? (
          <SpecialtyModal
            open
            selectedIds={currentSpecialtyIds}
            onClose={() =>
              setIsSpecialtyModalOpen(false)
            }
            onSaved={(updatedIds: string[]) => {
              setCurrentSpecialtyIds(updatedIds);
              setIsSpecialtyModalOpen(false);
            }}
          />
        ) : null}
