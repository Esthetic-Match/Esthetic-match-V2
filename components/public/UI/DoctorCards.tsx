...
export type DoctorCardCatalogueItem = {
  id: string;
  name: string;
};
...
export type DoctorCardData = {
  ...
  /*
   * These are now returned directly from
   * the database-backed doctor listing API.
   */
  specialties: DoctorCardCatalogueItem[];
  topThreeProcedures: DoctorCardCatalogueItem[];
...
export default function DoctorCards({
  ...
  /*
   * Prefer the doctor's top-three procedures.
   *
   * If the doctor has not selected any top-three
   * procedures, display their specialties instead.
   */
  const mainTags =
    doctor.topThreeProcedures?.length > 0
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col px-6 pb-6 pt-16">
        ...
        {/* ═════════════════════════════
            TOP PROCEDURES / SPECIALTIES

            Top-three procedures are preferred.
            Specialties are the fallback.
        ═════════════════════════════ */}
        ...
        visibleTags.length > 0 ? (
          <div className="mt-5 border-t border-[#CEB591]/20 pt-4">
            ...
            {/* Structured data:
                medicalSpecialty should contain actual specialties,
                not procedure names. */}

            {specialties.map(
              (specialty) => (
                <meta
                  key={`schema-${specialty.id}`}
