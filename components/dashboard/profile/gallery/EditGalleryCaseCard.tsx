...
export default function EditGalleryCaseCard({
  ...
  const t = useTranslations(
    "dashboard.editGallery.caseCard"
  );
  ...
  const isVisibilitySaving =
    visibilitySavingId === item.id;

  const selectedProcedureId =
    item.procedureId ?? "";

  const procedureLabel = selectedProcedureId
    ? tProcedures.has(selectedProcedureId)
      ? tProcedures(selectedProcedureId)
      : formatProcedureLabel(selectedProcedureId)
    : null;
    ...
    <>
      <article className="relative grid gap-6 rounded-[2rem] border border-[#283C5D]/10 bg-white p-4 shadow-sm md:grid-cols-[1.1fr_0.9fr] md:p-6">
        ...
        <div className="grid h-full gap-4 sm:grid-cols-2">
          <GalleryImage
            label={t("before")}
            imagePath={item.beforeImage}
          />

          <GalleryImage
            label={t("after")}
            imagePath={item.afterImage}
          />
        </div>
